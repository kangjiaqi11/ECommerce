        ...
        public RecommendPage RecommendShow(string GoodsName,int IsRecommend, int PageIndex, int PageSize)
        {
            ...
            if (SqlConnection != null)
            {
                ...
                while (reader.Read())
                {
                    ...
                    info.RecommendId = Convert.ToInt32(reader["ActivityId"]);
                    info.IsRecommend = Convert.ToString(reader["ActivityTitle"]);
                    info.GoodsName = Convert.ToInt32(reader["ActivityStatel"]);
                    info.GoodsIntro = Convert.ToInt32(reader["TimeQId"]);
                    info.GoodId = Convert.ToInt32(reader["GoodId"]);
                    list.Add(info);
                }
                reader.Close();
